...
const Dashboard = () => {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      // Apply filtering based on frequency and type
      if (type !== "all") {
        filteredData = filteredData.filter(expense => expense.category === type);
      }
  
      if (frequency !== "all") {
        filteredData = filteredData.filter(expense => {
          const expenseDate = new Date(expense.date); // Fix: Place within function scope
  
          if (frequency === "day") {
            return expenseDate.toDateString() === now.toDateString(); // Same day
          } else if (frequency === "week") {
            const weekAgo = new Date();
            weekAgo.setDate(now.getDate() - 7);
            return expenseDate >= weekAgo && expenseDate <= now; // Last 7 days
      ...
      setExpenses(filteredData); // Update state with filtered results
    } catch (error) {
    ...
    <div className={`dashboard-container ${darkMode ? "dark" : "light"}`}> 
      ...
      <div className="controls">
        <FormControl variant="outlined" size="small">
          ...
          <MenuItem value="all" className="custom-menu-item">All</MenuItem>
        ...
        <div>
          ...
          <Button variant={viewMode === "chart" ? "contained" : "outlined"} onClick={() => navigate("/charts")}>
            <FaChartPie />
          </Button>
        </div>
